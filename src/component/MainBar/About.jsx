...
import { FaShieldHalved, FaTerminal, FaNetworkWired, FaBug, FaChevronDown, FaChevronUp } from 'react-icons/fa6';
...
const About = () => {
  ...
  const preview = aboutData.bio[0];
  const rest = aboutData.bio.slice(1);
      ...
      <section className="space-y-5 text-text-secondary text-base leading-relaxed max-w-4xl">
        ...
        <AnimatePresence initial={false}>
          {expanded && rest.map((para, i) => (
            <motion.p
              ...
              exit={{ opacity: 0, y: 8 }}
              transition={{ duration: 0.32, delay: i * 0.07, ease: [0.215, 0.61, 0.355, 1] }}
            >
              {para}
            </motion.p>
          ))}
        ...
        >
          {expanded
            ...
            : <><FaChevronDown size={11} /> Show more</>
          }
        </motion.button>
      </section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
          {aboutData.techStack.map((s, i) => (
            <motion.a
              ...
              className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl group transition-all duration-200 hover:-translate-y-0.5"
              style={innerCard}
              ...
              <span className="text-[13px] font-medium text-text-secondary group-hover:text-text-primary transition-colors truncate">{s.label}</span>
            </motion.a>
          ))}
        </div>
      </section>
